  ...
  test() {
    ...
    try {
      ...
      if (matches.length > 0) {
        ...
        let lastIndex = 0;
        matches.forEach(m => {
          result += highlighted.substring(lastIndex, m.index);
          result += `<span class="bg-green-200 dark:bg-green-900/50 text-green-900 dark:text-green-100 font-bold px-1 rounded">${highlighted.substring(m.index, m.index + m.match.length)}</span>`;
